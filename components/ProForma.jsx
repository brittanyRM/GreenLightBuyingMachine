...
export default function ProForma({ deal, rooms = [], market = null, comps = [], orgRows = null, readOnly = false }) {
  ...
  const [ov, setOv] = useState({
    ...
    ltv: A.ltv,
    rate: rateForDown(1 - A.ltv, A.interest_rate, A),
    ...
    <div className="bg-neutral-100 p-4 font-sans sm:p-8">
      <div className="print-doc mx-auto max-w-4xl bg-white shadow-xl">
        ...
        <div className="border-b border-neutral-200 bg-neutral-50 px-6 py-2 text-[11px] leading-snug text-neutral-600 sm:px-8">
          ...
          until it fills.
          <br />
          Lender rates are current as of {asOfDate} — {pct(rateForDown(0.15, 0.065, A))} at
          15% down, {pct(rateForDown(0.2, 0.065, A))} at 20%, {pct(rateForDown(0.25, 0.065, A))} at 25%.
            ...
            <Field
              ...
              onChange={(v) => {
                ...
                setOv((prev) => ({
                  ...
                  rate: rateEdited ? prev.rate : rateForDown(1 - ltv, A.interest_rate, A),
                }));
              }}
            />
            <Field
              ...
              hint={
                rateEdited
                  ? `overridden — ${Math.round((1 - ov.ltv) * 100)}% down normally prices at ${pct(rateForDown(1 - ov.ltv, 0.065, A))}`
                  : `follows the down payment — ${Math.round((1 - ov.ltv) * 100)}% down prices at ${pct(rateForDown(1 - ov.ltv, 0.065, A))}`
              }
            />
