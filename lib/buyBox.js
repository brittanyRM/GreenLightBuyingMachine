...
/**
 * @returns {{ matches: boolean, failures: string[], nearMisses: string[], checked: number }}
...
export function matchBuyBox(deal = {}, box = null, metrics = null) {
  if (!box || box.active === false) {
    return { matches: true, failures: [], nearMisses: [], checked: 0 };
  }
  ...
  if (metrics) {
    bound(metrics.dscr, box.min_dscr, `DSCR under ${box.min_dscr}`, false);
    bound(metrics.capRate, box.min_cap_rate, `Cap rate under ${box.min_cap_rate}%`, false);
  }
...
export function describeBuyBox(box) {
  ...
  if (has(box.min_dscr)) bits.push(`DSCR ${box.min_dscr}+`);
