...
export async function getBuyerFromRequest(req) {
  ...
  const { data: user } = await admin()
    .from("buyer_users")
    .select("id, org_id, email, name, active, buyer_orgs!inner(id, name, slug, active)")
  ...
  return {
    ...
    email: user.email,
    name: user.name,
    org: { id: user.buyer_orgs.id, name: user.buyer_orgs.name, slug: user.buyer_orgs.slug },
  };
}
...
export const BUYER_DEAL_FIELDS = [
  ...
  "list_price",
  "hero_image_url",
  // The rendered plan the flyer publishes, not the raw dimensioned
  // sketch — floor_plan_url is a working drawing and shouldn't go to
  // a buyer.
