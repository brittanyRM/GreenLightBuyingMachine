...
function rateGroups(rooms, market, overrides, deal) {
  const groups = {};
  rooms
    /* record-resolved upstream */
    .forEach((r) => {
      const rate = roomRate(r, market, overrides, deal);
...
export function buildDealEmail({
  ...
  const groups = rateGroups(resolveRooms(rooms, deal), market, overrides, deal);
