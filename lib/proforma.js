...
export function roomMix(rooms = [], deal = {}) {
  ...
  const targetBaths = Number(deal.target_bathrooms) || 0;
  const bathrooms = targetBaths
    ? (ensuites && ensuites <= targetBaths
        ? targetBaths
        : targetBaths + ensuites)
    : fromSchedule > 0
    ? fromSchedule
    : fromRecord;
  ...
  return {
    ...
    bathrooms,
    commonBathrooms: targetBaths || common,
