...
export function pppCostForDown(downPct, fallback = 15000) {
...
export function computeProForma({
  ...
  const pppCost = Number(overrides.pppCost ?? pppCostForDown(1 - ltv));
  ...
  const financingOptions = [0.15, 0.2, 0.25].map((downPct) => {
    ...
    const oPppCost = Number(overrides.pppCost ?? pppCostForDown(downPct));
