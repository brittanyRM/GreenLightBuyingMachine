...
//
// Homeowners Financial Group (Rachelle Coffey, NMLS #203664), quoted
// 4-8 September 2026 on a $500,000 loan. All 30-year fixed — the tiers
// differ by rate, not by term.
//
// FICO 700 throughout. There are FICO 740 quotes at 80 and 85 LTV
// which price 0.5-0.6% better, but none at 75 LTV, so using them would
// mean the 25% tier sat on a different credit profile from the other
// two and the three columns would stop being comparable.
//
// These are the 3%-fixed prepayment structure: the same rate as the
// 6-month-interest option but a lower penalty cost, and materially
// cheaper than the 5%-fixed option's $25,000. See PPP_COST_BY_DOWN.
// Keys in org_assumptions that override the tiers below, so a rate
// change is a settings edit rather than a code change. Named here so
// the settings form, the public endpoint and the engines all use the
// same strings — three places typing "rate_15_down" by hand is how
// they drift.
...
// What the prepayment penalty costs to buy, by tier.
//
// New. These loans carry a prepayment penalty and the option chosen
// has a price — $13,750 to $25,000 — payable at close. It was absent
// from the model entirely, so cash to close was understated by roughly
// the origination fee again.
export const PPP_COST_BY_DOWN = {
  0.15: 15000,
  0.2: 15000,
  0.25: 15000,
};

// The alternatives, kept so the quote can be reconstructed rather than
// taken on trust. Rate first, penalty cost second.
export const LENDER_QUOTE_2026_09 = {
  asOf: "2026-09-08",
  lender: "Homeowners Financial Group · Rachelle Coffey NMLS #203664",
  term: "30-year fixed",
  fico: 700,
  tiers: {
    0.25: { ltv: 0.75, sixMoInterest: [0.06875, 13750], fivePctFixed: [0.0675, 25000], threePctFixed: [0.06875, 15000] },
    0.2: { ltv: 0.8, sixMoInterest: [0.07625, 15250], fivePctFixed: [0.075, 25000], threePctFixed: [0.07625, 15000] },
    0.15: { ltv: 0.85, sixMoInterest: [0.08875, 17750], fivePctFixed: [0.08625, 25000], threePctFixed: [0.08875, 15000] },
  },
};

// As rateForDown, for the penalty cost.
// Tiers with any org overrides applied. Pass the merged org row and
// the numbers come from settings; pass nothing and you get the built-in
// quote, which is what the fallbacks are for.
export function rateTiers(org = null) {
  const out = {};
  for (const [down, key] of Object.entries(RATE_KEYS)) {
    const v = org && org[key] != null ? Number(org[key]) : null;
    out[down] = Number.isFinite(v) && v > 0 ? v : RATE_BY_DOWN[down];
  }
  return out;
}

export function pppCostForDown(downPct, fallback = 15000, org = null) {
  const override = org && org[PPP_KEY] != null ? Number(org[PPP_KEY]) : null;
  if (Number.isFinite(override) && override >= 0) return override;
  const tiers = Object.keys(PPP_COST_BY_DOWN)
    .map(Number)
    .sort((a, b) => a - b);
  let picked = null;
  for (const t of tiers) if (downPct >= t - 1e-9) picked = t;
  return picked != null ? PPP_COST_BY_DOWN[picked] : fallback;
}

// Nearest tier at or below the down payment, so an unusual figure
// still prices rather than falling back to a single default.
// org is the merged assumptions row. Passing it makes the rate come
// from settings; omitting it falls back to the built-in quote, which
// keeps every existing caller working unchanged.
export function rateForDown(downPct, fallback = 0.065, org = null) {
  const table = rateTiers(org);
  const tiers = Object.keys(table)
    .map(Number)
    .sort((a, b) => a - b);
  let picked = null;
  for (const t of tiers) if (downPct >= t - 1e-9) picked = t;
  return picked != null ? table[picked] : table[tiers[0]] ?? fallback;
}
...
//
//   1. a rate set on the room itself, for a room that differs
//   2. a live override from the sheet's rate boxes
//   3. THE RECORD — shared_weekly_rate / ensuite_weekly_rate
//   4. the ZIP market row
//
// The record sits above the market because a particular house is
// priced on its own merits, not on its postcode. It sits below a
// per-room rate because that's the point of a per-room rate.
export function roomRate(room, market = {}, overrides = {}, deal = {}) {
  ...
  if (room.room_type === "ensuite") {
    return Number(
      overrides.ensuiteRate ??
        deal.ensuite_weekly_rate ??
        market.private_weekly ??
        0
    );
  }
  return Number(
    overrides.sharedRate ?? deal.shared_weekly_rate ?? market.shared_weekly ?? 0
  );
}
...
// The bedroom count to show in a list.
//
// The target is what the house is being underwritten and sold as; the
// as-is count is what the assessor recorded before conversion. Lists
// were reading the as-is one, so a 7-bedroom conversion with no
// pre-conversion figure on file displayed as "?" while its own pro
// forma priced seven rooms.
export function displayBedrooms(deal = {}) {
  return Number(deal.target_bedrooms) || Number(deal.bedrooms) || null;
}
...
// The rentable rooms this deal is underwritten to.
//
// The record decides how many and of what kind. Drawn rooms supply the
// detail — labels, per-room rate overrides, ensuite pairings — but
// never the count. A sketch is a work in progress and a half-traced
// plan must not reprice the deal.
//
// Everything downstream reads this: the mix, the income, the flyer,
// the pro forma. One resolution, one answer.
export function resolveRooms(rooms = [], deal = {}) {
  const drawn = (rooms || []).filter(
    (r) => r.room_type === "shared" || r.room_type === "ensuite"
  );

  const targetBeds = Number(deal.target_bedrooms) || 0;
  const target = targetBeds || drawn.length;
  if (!target) return [];

  const targetEnsRaw = Number(deal.target_ensuites);
  const targetEns = Number.isFinite(targetEnsRaw) && targetEnsRaw >= 0
    ? targetEnsRaw
    : drawn.filter((r) => r.room_type === "ensuite").length;

  // Drawn ensuites first so their rates and pairings survive, then
  // drawn shared, then whatever the record still expects.
  const drawnEns = drawn.filter((r) => r.room_type === "ensuite");
  const drawnShared = drawn.filter((r) => r.room_type === "shared");

  const out = [];
  for (let i = 0; i < Math.min(targetEns, target); i++) {
    out.push(
      drawnEns[i] || {
        id: `record-e${i}`,
        room_number: out.length + 1,
        room_type: "ensuite",
        label: `Bedroom ${out.length + 1}`,
      }
    );
  }
  let si = 0;
...
export function roomMix(rooms = [], deal) {
  // The deal is required. Calling this with rooms alone counts what's
  // drawn and ignores target_bedrooms / target_bathrooms entirely —
  // which is how the flyer printed 7 bathrooms while the header
  // printed 5 from this same function. Defaulting to {} made that
  // failure silent, so it doesn't default any more.
  if (!deal) {
    if (typeof console !== "undefined") {
      console.warn("roomMix called without a deal — record counts ignored.");
    }
    deal = {};
  }
  ...
  // target_bathrooms is the TOTAL, and target_ensuites says how many
  // of that total are ensuite. The field reads "Of those, ensuites" —
  // "of those" means a subset, and that's how it gets filled in.
  //
  // I had this the other way for a while, treating the field as common
  // baths and adding ensuites on top. That produced 7 on a house with
  // 5 total, and disagreed with the badge on the same form.
  const targetBaths = Number(deal.target_bathrooms);
  const bathrooms = Number.isFinite(targetBaths) && targetBaths > 0
    ? targetBaths
    : bathRows > 0
    ? bathRows + ensuites
    : common + ensuites;

  const commonBaths = Math.max(0, bathrooms - ensuites);
...
export function computeProForma({
  ...
  const rate = Number(overrides.rate ?? rateForDown(downPct, A.interest_rate, A));
  ...
  // Priced off the record's room count, not the sketch's.
  const rentable = resolveRooms(rooms, deal);
  const grossWeekly = rentable.reduce(
    (sum, r) => sum + roomRate(r, market, overrides, deal),
    0
  );
  ...
  // Prepayment penalty, bought at close. See PPP_COST_BY_DOWN.
  const pppCost = Number(overrides.pppCost ?? pppCostForDown(1 - ltv, 15000, A));
  const cashIn = down + origination + closingCosts + pppCost;
  ...
  const financingOptions = [0.15, 0.2, 0.25].map((downPct) => {
    ...
    const oPppCost = Number(overrides.pppCost ?? pppCostForDown(downPct, 15000, A));
    const oCashIn = oDown + oOrigination + closingCosts + oPppCost;
    // Each tier prices differently — that's most of the trade-off.
    const oRate = Number(overrides.rate ?? rateForDown(downPct, A.interest_rate, A));
