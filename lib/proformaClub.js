...
import { usd, pct, rateForDown, pppCostForDown } from "./proforma";
...
export function downPaymentOptions({
  ...
  // Merged org assumptions, so the tiers come from settings rather
  // than the built-in quote. Optional: without it the fallbacks apply
  ...
  return downPcts.map((downPct) => {
    ...
    // The prepayment penalty is bought at close, so it belongs in cash
    // in. Leaving it out understated what a buyer actually writes a
    // cheque for by about the size of the origination fee again, and
    // overstated cash-on-cash by dividing by too small a number.
    const pppCost = pppCostForDown(downPct, 15000, org);
    const cashIn = down + origination + closingCosts + pppCost;

    const rate = rateForDown(downPct, 0.065, org);
