import { resolveRooms, rateForDown } from "./proforma";
// ============================================================
...
export function inputsFromDeal({ deal = {}, rooms = [], market = null, org = null } = {}, opts = {}) {
  ...
  return {
    ...
    debt: {
      // Priced off the down payment, the same as every other surface.
      //
      // This read org_assumptions.interest_rate, which defaults to
      // 6.5% — the rate before Rachelle's September quote. So the buyer
      // sheet computed debt service at 6.5% while the pro forma, the
      // flyer and the lender package all used 6.875%, and the coverage
      // ratios disagreed for a reason that had nothing to do with the
      // expense model everyone was blaming.
      //
      // An explicit rate on the deal still wins; only the fallback
      // changes, from a stale constant to the current tier.
      interestRatePct:
        orgValue(org, "interest_rate", null, deal.assumption_overrides || null) != null
          ? orgValue(org, "interest_rate", 0.065, deal.assumption_overrides || null) * 100
          : rateForDown(
              1 - orgValue(org, "ltv", 0.75, deal.assumption_overrides || null),
              0.065,
              org
            ) * 100,
      interestOnly: false,
...
// The worked-example preset lived here. Removed with the demo it fed:
// a fictional property confused people about whether it was real, and
// pointing it at a real one confused them about which page they were
// on. Deals come from the deals table.
