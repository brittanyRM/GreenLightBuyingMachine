import { resolveRooms } from "./proforma";
// ============================================================
...
// Resolution order: what this house actually costs, then the house
// standard, then a built-in default. A blank override is not zero —
// it means "use the standard".
function orgValue(org, key, fallback, overrides = null) {
...
export function buildColivingExpenses(bedCount, overrides = {}, org = null, deal = {}) {
  const ov = deal.assumption_overrides || null;
  const perRoomMonthly = orgValue(org, "opex_per_room", 110, ov);
  ...
  // than with income.
  const maintRate = orgValue(org, "maintenance_rate", 0.02, ov);
  ...
  const taxes = Number(ov?.property_taxes_annual)
    ? Number(ov.property_taxes_annual)
    : assessed
    ? Math.round(assessed * orgValue(org, "tax_reclass_factor", 2.35, ov))
    : 3000;

  return {
    propertyTaxesAnnual: taxes,
    insuranceAnnual: orgValue(org, "insurance_annual", 2400, ov),
    hoaAnnual: 0,
    ...
    // No org_assumptions equivalent, so these are per-deal only.
    turnoverAnnual: Number(ov?.turnover_annual) || 0,
    repairsMaintenanceAnnual: Number(ov?.repairs_annual) || 0,
...
export function inputsFromDeal({ deal = {}, rooms = [], market = null, org = null } = {}, opts = {}) {
  ...
  // Record first, then the ZIP. The old hardcoded fallbacks were 85201
  // figures and quietly mispriced every other market.
  const sharedWeekly =
    Number(deal.shared_weekly_rate) || Number(market?.shared_weekly) || 0;
    const privateWeekly =
    Number(deal.ensuite_weekly_rate) || Number(market?.private_weekly) || 0;

  // Same resolver the deal-page pro forma uses: the record decides how
  // many rooms exist, the sketch supplies their detail.
  const rentable = resolveRooms(rooms, deal);

  // No room schedule yet: build from the record's own counts rather
  // than a fixed 8-and-1. The old fallback invented nine rooms on an
  // eight-bed house, so the masthead and the income table disagreed.
  const synthesised = [
  ...
  const built = rentable.length
    ...
    ? synthesised.map((r, i) => ({
        id: r.id,
        label: r.room_type === "ensuite" ? `Ensuite ${i + 1}` : `Bedroom ${r.room_number}`,
        weeklyRate: r.room_type === "ensuite" ? privateWeekly : sharedWeekly,
        isEnsuite: r.room_type === "ensuite",
      }))
  ...
  return {
    ...
    capitalization: {
      ...
      ltv: orgValue(org, "ltv", 0.75, deal.assumption_overrides || null),
      // closing_costs is stored as a dollar figure, so convert. Values
      // under 1 are already a fraction.
      closingCostPct: (() => {
        const cc = orgValue(org, "closing_costs", null, deal.assumption_overrides || null);
        if (cc === null) return 0.01;
        return cc > 1 ? (price ? cc / price : 0.01) : cc;
      })(),
      loanCostPct: orgValue(org, "origination_points", 0.015, deal.assumption_overrides || null),
    ...
    debt: {
      interestRatePct: orgValue(org, "interest_rate", 0.065, deal.assumption_overrides || null) * 100,
      interestOnly: false,
      amortizationMonths: orgValue(org, "loan_term_years", 30, deal.assumption_overrides || null) * 12,
    },
    ...
    scenarios: {
      ...
      glbm: {
        income: {
          rooms: built,
          occupancyPct: 1 - orgValue(org, "vacancy_rate", 0.05, deal.assumption_overrides || null),
          collectionsPct: 0.97,
          platform: PADSPLIT_FEES,
          growthPct: 0.03,
        },
        expenses,
        exit: { appreciationPct: orgValue(org, "appreciation_rate", 0.03, deal.assumption_overrides || null) },
      },
      bear: {
        income: {
          rooms: built,
...
export function toTemplateLens(inputs, monthlyMisc = 1000) {
  ...
  return {
    ...inputs,
    scenarios: {
      // GLBM carries through the template lens too, keeping the same
      // occupancy and only swapping the expense stack — otherwise
      // selecting GLBM with the lens on would ask for a case that
      // doesn't exist.
      ...(inputs.scenarios.glbm
        ? { glbm: { ...inputs.scenarios.glbm, expenses: flat } }
        : {}),
...
// The model deal — 1025 E Dolphin Ave, Mesa AZ 85204.
//
// 9 bed / 4 bath with 2 ensuites, at a price close to what Green Light
// actually buys at. This replaced an invented property at a made-up
// address, which was fine for checking a layout and useless for
// showing anyone what the model does.
//
// Figures here are a fallback. MODEL_DEAL_SLUG is the real record, and
// the demo route loads it when it can — a worked example that quietly
// diverges from the house it claims to describe is worse than no
// example. Everything stays adjustable either way: the assumptions
// panel edits this the same as any other deal.
export const MODEL_DEAL_SLUG = "1025-n-dolphin-ave-mesa";

export const MODEL_DEAL_FALLBACK = {
  deal: {
    address_line: "1025 E Dolphin Ave",
    city: "Mesa",
    state: "AZ",
    zip: "85204",
    county: "Maricopa",
    parcel_number: "139-20-073",
    subdivision: "Laurel Park 2",
    school_district: "Mesa Unified School District",
    year_built: 1964,
    lot_sqft: 8856,
    // No square footage on this one. The assessor figure predates the
    // conversion and nothing has been measured since, so the sheet
    // prices per bedroom rather than per foot. Inventing a number to
    // fill the column would be the wrong fix.
    target_bedrooms: 9,
    target_bathrooms: 4,
    target_ensuites: 2,
    bathrooms: 4,
    shared_weekly_rate: 205,
    ensuite_weekly_rate: 297,
    list_price: 545000,
  },
  rooms: [],
  market: {
    ...
    private_weekly: 297,
    avg_occupancy: 0.95,
    days_to_first_booking: 4,
    days_to_80_booked: 28,
  },
};

export function modelDealInputs(source = null) {
  const src = source || MODEL_DEAL_FALLBACK;
...
// Old name, kept so nothing breaks on an import that hasn't been
// updated. Points at the same model.
export const pepperPlaceInputs = modelDealInputs;
