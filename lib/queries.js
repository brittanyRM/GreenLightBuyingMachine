...
const compKey = (c) =>
  c.mls_number
    ? `mls:${String(c.mls_number).replace(/\D/g, "")}`
...
export async function saveComps(dealId, comps, { replace = false } = {}) {
  ...
  const prior = new Map((existing || []).map((c) => [compKey(c), c]));

  const toInsert = [];
  const toUpdate = [];
  for (const [key, c] of incoming) {
    const found = prior.get(key);
    if (!found) {
