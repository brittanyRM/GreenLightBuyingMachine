...
export async function getDealBundle(slug) {
  ...
  const [rooms, comps, market, org, docs, settings, report] = await Promise.all([
    ...
    // The city report the buyer sheet renders. Loaded here so the deal
    // page can show the same thing rather than a second view of it —
    // the internal side should see what a buyer sees, not an
    // approximation of it.
...
//
// Unused since the /p/ share route was removed. There were two ways to
// share a pro forma — /p/ from the deal page and /s/ from the buyer
// sheet — producing two different-looking documents from two token
// schemes. Nothing had been sent through either yet, so the duplicate
// went rather than being kept alive.
//
// These are left in place, not deleted: pro_forma_snapshots still
// holds rows, send-deal still stamps them, and a reader tracing that
// table wants to find the code that wrote it.
// ============================================================
